...
@interface EValueteController ()<DropDownMwnuDelegate,ClassMenuDelegate,PersonMenuDelegate,SecondDropMenuDelegate,UITextViewDelegate,UITextFieldDelegate,GYZCustomCalendarPickerViewDelegate,YYTextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,SDPhotoBrowserDelegate,TZImagePickerControllerDelegate>
...
- (void)viewDidLoad {
    ...
    _popTextView = [YYTextView new];
    
    _record.layer.borderColor = [UIColor lightGrayColor].CGColor;
    _record.layer.borderWidth = .5f;
    ...
    self.teacherFeatureTF.delegate = self;
...
//    }
    ...
    NSData *datatt = [[NSUserDefaults standardUserDefaults]objectForKey:@"dataTempSave"];
    if (datatt.length) {
        YYTextLayout *layout = [NSKeyedUnarchiver unarchiveObjectWithData:datatt];
        _record.attributedText = layout.text;
        _popTextView.attributedText = layout.text;
    }
...
- (void) textViewDidChange:(UITextView*)textview
{
    if ([textview isEqual:_popTextView] ) {
        ...
        NSMutableAttributedString *attrting =[[NSMutableAttributedString alloc]initWithAttributedString: _popTextView.attributedText];
        [attrting addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:17] range:NSMakeRange(0, _popTextView.text.length)];

//        dispatch_async(dispatch_queue_create("async.saveTempData", DISPATCH_QUEUE_SERIAL), ^{
//            //异步,串行
//            NSData *data = [NSKeyedArchiver archivedDataWithRootObject:_popTextView.textLayout];
//            [[NSUserDefaults standardUserDefaults]setObject:data forKey:@"dataTempSave"];
...
{
    if ([textField isEqual:_mainItem]) {
    ...
    else if([textField isEqual:_subjectTF])
    {
        NSString * toBeString = [textField.text stringByReplacingCharactersInRange:range withString:string];
        [[NSUserDefaults standardUserDefaults]setValue:toBeString forKey:@"subjectInput"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        return YES;
    }
    else if([textField isEqual:_teacherFeatureTF])
    {
        NSString * toBeString = [textField.text stringByReplacingCharactersInRange:range withString:string];
        [[NSUserDefaults standardUserDefaults]setValue:toBeString forKey:@"teacherFeature"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        return YES;
    }
    return YES;
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    {
        [[NSUserDefaults standardUserDefaults]setObject:textField.text forKey:@"subjectInput"];
        [[NSUserDefaults standardUserDefaults]synchronize];
    }
    else if([textField isEqual:_teacherFeatureTF])
    {
        [[NSUserDefaults standardUserDefaults]setObject:textField.text forKey:@"teacherFeature"];
        [[NSUserDefaults standardUserDefaults]synchronize];
    }
...
- (IBAction)clearButtonClickedAction:(id)sender {
    
    _tapArray = [NSMutableArray array];
    self.chooseCountLabel.text = [NSString stringWithFormat:@"已选择:%lu",(unsigned long)_tapArray.count];
    
    [_evaluateLevelArray removeAllObjects];
    [self.lifeTableView reloadData];
    [self.deadTableView reloadData];
...
- (IBAction)beginMark:(id)sender {
    ...
    main.subject = self.subjectTF.text;
    main.teacherFeature = self.teacherFeatureTF.text;
    ...
    {
        //获取图片资源
        main.textviewImageArray = [NSMutableArray arrayWithArray:[self getLocationImageWithText:_record]];
        
        //获取图片位置
        NSMutableAttributedString *attring = [[[NSMutableAttributedString alloc]initWithAttributedString:_record.attributedText] mutableCopy];
        YYTextView *textview = [YYTextView new];
        textview.attributedText = attring;
        textview.text = [self insertLocationStringToText:textview];
        //emojiEncode Deal
        main.content = [self emojiEncode:textview.attributedText];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *subjectString =[[NSUserDefaults standardUserDefaults] objectForKey:@"subjectInput"];
    if (![self isBlankString: subjectString]) {
        _subjectTF.text = subjectString;
    }
    NSString *teacherFeatureString =[[NSUserDefaults standardUserDefaults] objectForKey:@"teacherFeature"];
    if (![self isBlankString: teacherFeatureString]) {
        _teacherFeatureTF.text = teacherFeatureString;
    }
...
- (IBAction)showBigTextViewAction:(id)sender {
    ...
    _popTextView.font = [UIFont systemFontOfSize:17];
...
//    _popTextView.placeholder = @"教学过程简要记录";
    ...
    _popTextView.placeholderText = @"教学过程简要记录";
    
    _popTextView.layer.borderWidth = 1.f;
    
    NSMutableAttributedString *attriStr = [_popTextView.attributedText mutableCopy];
    [attriStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:17] range:NSMakeRange(0, _popTextView.text.length)];
    _popTextView.attributedText = attriStr;
    
    NSMutableArray *arrat = [self getLocationImageWithText:_popTextView];
    ...
    _popTextviewRight.font = [UIFont systemFontOfSize:17];
...
- (void)dismiss:(UIButton *)btn {
    ...
    _record.attributedText = _popTextView.textLayout.text;
//    dispatch_async(dispatch_queue_create("async.saveTempData", DISPATCH_QUEUE_SERIAL), ^{
        //异步,串行
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:_popTextView.textLayout];
        [[NSUserDefaults standardUserDefaults]setObject:data forKey:@"dataTempSave"];
...
-(void)AddPictureAction
{
    ...
    [alert addAction: [UIAlertAction actionWithTitle: @"从相册选取" style: UIAlertActionStyleDefault handler:^(UIAlertAction *action){
        ...
        TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:1 columnNumber:8 delegate:self pushPhotoPickerVc:YES];
        ...
        [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
            ...
            NSMutableAttributedString *text = [_popTextView.attributedText mutableCopy];
            // 1.2 获取光标位置
            NSRange rg = _popTextView.selectedRange;
            //        https://www.jianshu.com/p/34d14d928395
            
            YYImage *image = [YYImage imageWithData:[NSData dataWithData:[self zipNSDataWithImage:photos.lastObject]]];
            image.preloadAllAnimatedImageFrames = YES;        //添加文本+图片
            [text appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n" attributes:nil]];
            {
                
                YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithImage:image];
                imageView.frame = CGRectMake(0, 0,self.popTextView.frame.size.width - 48, ((self.popTextView.frame.size.width - 48)/ photos.lastObject.size.width) * photos.lastObject.size.height);
                imageView.userInteractionEnabled = YES;
                UITapGestureRecognizer *ges = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(locationImageviewTapAction:)];
                [imageView addGestureRecognizer:ges];
                
                
                NSMutableAttributedString *attachText = [NSMutableAttributedString yy_attachmentStringWithContent:imageView contentMode:UIViewContentModeScaleAspectFit attachmentSize:imageView.size alignToFont:[UIFont systemFontOfSize:17] alignment:YYTextVerticalAlignmentCenter];
                [text insertAttributedString:attachText atIndex:rg.location];
                
//                NSString *nString = [NSString stringWithFormat:@" \n"];
//                NSAttributedString *nAddString = [[NSAttributedString alloc]initWithString:nString];
//                [text appendAttributedString:nAddString];
            }
            [text yy_setFont:[UIFont systemFontOfSize:17] range: text.yy_rangeOfAll];
            _popTextView.attributedText = text;
            [_popTextView becomeFirstResponder];
            _popTextView.selectedRange = NSMakeRange(_popTextView.attributedText.length, 1);
            
//            _popTextView.selectable = NO;
//            _popTextView.selectable = YES;
//
//            NSRange range;
//            range.location = _popTextView.attributedText.length;
//            range.length = 0;
//            _popTextView.selectedRange = range;
...
//        https://www.jianshu.com/p/34d14d928395
        ...
        YYImage *image = [YYImage imageWithData:[NSData dataWithData:[self zipNSDataWithImage:img]]];
        image.preloadAllAnimatedImageFrames = YES;        //添加文本+图片
        [text appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n" attributes:nil]];
        {
            
            YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithImage:image];
            imageView.frame = CGRectMake(0, 0,self.popTextView.frame.size.width - 48, ((self.popTextView.frame.size.width - 48)/ img.size.width) * img.size.height);
            imageView.userInteractionEnabled = YES;
            UITapGestureRecognizer *ges = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(locationImageviewTapAction:)];
            [imageView addGestureRecognizer:ges];
...
//                                   };
//    NSData *htmlData = [string dataFromRange:NSMakeRange(0, string.length) documentAttributes:exportParams error:nil];
...
#pragma mark - dealAttString
//获取POPTEXTVIEW的处理后带attatchment位置的文本
-(NSString *)insertLocationStringToText:(YYTextView *)textview
{
    NSMutableString *muString = [[NSMutableString stringWithString:textview.text] mutableCopy];
    
    NSArray *attachmentRanges = _record.textLayout.attachmentRanges;
    
    NSInteger idx = 0;
    for (NSValue *range in attachmentRanges) {
        NSString *string = [NSString stringWithFormat:@"Attachment%lu.png",(unsigned long)idx];
        [muString insertString:string atIndex:[range rangeValue].location + string.length * idx];
        idx++;
    }
    
//    [attachmentRanges enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSValue *range, NSUInteger idx, BOOL * _Nonnull stop) {
//
//    }];
//    [attachmentRanges enumerateObjectsUsingBlock:^(NSValue *range, NSUInteger idx, BOOL * _Nonnull stop) {
//        NSString *string = [NSString stringWithFormat:@"Attachment%lu.png",(unsigned long)idx];
//        [muString insertString:string atIndex:[range rangeValue].location];
//    }];
...
//获取POPTEXTVIEW的富文本图片
-(NSMutableArray *)getLocationImageWithText:(YYTextView *)textview
{
    NSArray *attachments =  _record.textLayout.attachments;
    
    NSMutableArray *locatinImageArray = [NSMutableArray array];
    for(YYTextAttachment *attachment in attachments)
    {
        ...
        [locatinImageArray addObject: image];
        
        imageView.userInteractionEnabled = YES;
        UITapGestureRecognizer *ges = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(locationImageviewTapAction:)];
        [imageView addGestureRecognizer:ges];
    }
    return locatinImageArray;
}


//t富文本图片点击浏览
-(void)locationImageviewTapAction:(UITapGestureRecognizer *)sender
{
    YYAnimatedImageView *anView = (YYAnimatedImageView *)sender.view;
    showImage = anView.image;
